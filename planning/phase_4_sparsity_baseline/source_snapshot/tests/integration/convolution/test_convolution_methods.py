...
from PLD_accounting.discrete_dist import GeneralDiscreteDist, DenseGeometricDiscreteDist, DenseLinearDiscreteDist, SparseLinearDiscreteDist
...
class TestGeometricConvolution:
    ...
    def test_convolve_two_distributions(self):
        ...
        from PLD_accounting.discrete_dist import DenseGeometricDiscreteDist
        x1 = np.array([1.0, 2.0, 4.0])
        pmf1 = np.array([0.3, 0.5, 0.2], dtype=np.float64)
        dist1 = DenseGeometricDiscreteDist.from_x_array(x_array=x1, PMF_array=pmf1)

        x2 = np.array([0.5, 1.0])
        pmf2 = np.array([0.6, 0.4], dtype=np.float64)
        dist2 = DenseGeometricDiscreteDist.from_x_array(x_array=x2, PMF_array=pmf2)
    ...
    def test_self_convolution_t2(self):
        ...
        dist = DenseGeometricDiscreteDist.from_x_array(x_array=x, PMF_array=pmf)
...
def test_allocation_pmf_methods_dominate(direction, method):
    ...
    def test_fft_rejects_sparse_linear_input(self):
        """FFT path accepts only dense linear distributions."""
        ...
        with pytest.raises(TypeError, match="DenseLinearDiscreteDist"):
            FFT_self_convolve(
                dist=dist,
                T=2,
                tail_truncation=0.0,
