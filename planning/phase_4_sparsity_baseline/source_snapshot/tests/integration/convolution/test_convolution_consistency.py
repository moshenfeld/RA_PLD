...
from PLD_accounting.discrete_dist import SparseGeometricDiscreteDist
...
class TestConvolutionConsistency:
    ...
    def test_sparse_geometric_matches_dense_frame_equivalent(self):
        ratio = 1.01
        x_min_1 = 0.1
        x_min_2 = x_min_1 * (ratio ** 3)

        idx_1 = np.array([5, 11, 13, 40, 55], dtype=np.int64)
        idx_2 = np.array([2, 7, 20, 31, 42], dtype=np.int64)
        pmf_1 = np.array([0.18, 0.22, 0.14, 0.31, 0.15], dtype=np.float64)
        pmf_2 = np.array([0.10, 0.20, 0.25, 0.30, 0.15], dtype=np.float64)

        sparse_1 = SparseGeometricDiscreteDist(
            x_min=x_min_1,
            ratio=ratio,
            indices=idx_1,
            PMF_array=pmf_1,
        )
        sparse_2 = SparseGeometricDiscreteDist(
            x_min=x_min_2,
            ratio=ratio,
            indices=idx_2,
            PMF_array=pmf_2,
        ...
        sparse_out = geometric_convolve(
            dist_1=sparse_1,
            dist_2=sparse_2,
            tail_truncation=0.0,
            bound_type=BoundType.DOMINATES,
        )
        dense_out = geometric_convolve(
            dist_1=to_dense_geometric(sparse_1),
            dist_2=to_dense_geometric(sparse_2),
            tail_truncation=0.0,
            bound_type=BoundType.DOMINATES,
        )

        sparse_as_dense = to_dense_geometric(sparse_out)
        dense_as_dense = to_dense_geometric(dense_out)

        assert np.allclose(sparse_as_dense.x_array, dense_as_dense.x_array, atol=1e-12)
        assert np.allclose(sparse_as_dense.PMF_array, dense_as_dense.PMF_array, atol=1e-12)

    def test_sparse_geometric_large_scale_matches_dense_frame_equivalent(self):
        ratio = 1.01
        x_min_1 = 0.1
        x_min_2 = x_min_1 * (ratio ** 1200)

        idx_1 = np.array([0, 7, 22, 65, 130, 180], dtype=np.int64)
        idx_2 = np.array([3, 12, 19, 28, 44, 63], dtype=np.int64)
        pmf_1 = np.array([0.10, 0.18, 0.24, 0.22, 0.16, 0.10], dtype=np.float64)
        pmf_2 = np.array([0.11, 0.19, 0.17, 0.21, 0.18, 0.14], dtype=np.float64)
