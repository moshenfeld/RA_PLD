...
- LinearDiscreteDistBase: abstract linear-grid base (stores x_min, x_gap)
  - DenseLinearDiscreteDist: dense linear support
  - SparseLinearDiscreteDist: sparse linear support with explicit integer indices
- GeometricDiscreteDistBase: abstract geometric-grid base (stores x_min, ratio)
  - DenseGeometricDiscreteDist: dense geometric support
  - SparseGeometricDiscreteDist: sparse geometric support with explicit integer indices
"""
...
# =============================================================================
# STRUCTURED LINEAR DISTRIBUTIONS
# =============================================================================


class LinearDiscreteDistBase(DiscreteDistBase, ABC):
    """Abstract base for linear grids x[k] = x_min + x_gap * index[k]."""
    ...
    @abstractmethod
    def get_indices(self) -> NDArray[np.int64]:
        """Return integer indices that map support to x_min + x_gap * indices."""

    def get_x_array(self) -> NDArray[np.float64]:
        return self.x_min + self.get_indices().astype(np.float64) * self.x_gap

    def get_support_size(self) -> int:
        return self.get_indices().size


class DenseLinearDiscreteDist(LinearDiscreteDistBase):
    """Dense linear grid x[i] = x_min + i * x_gap."""
    ...
    def __init__(
        ...
        super().__init__(x_min=x_min, x_gap=x_gap, PMF_array=PMF_array, p_neg_inf=p_neg_inf, p_pos_inf=p_pos_inf)
    ...
    def from_x_array(
        ...
        """Create DenseLinearDiscreteDist from x_array by extracting x_min and x_gap.
        ...
        Returns:
            DenseLinearDiscreteDist instance
    ...
    def get_indices(self) -> NDArray[np.int64]:
        return np.arange(self.PMF_array.size, dtype=np.int64)
    ...
    def copy(self) -> DenseLinearDiscreteDist:
        return DenseLinearDiscreteDist(
            x_min=self.x_min,
            x_gap=self.x_gap,
            PMF_array=self.PMF_array.copy(),
...
class SparseLinearDiscreteDist(LinearDiscreteDistBase):
    """Sparse linear grid x[k] = x_min + x_gap * indices[k]."""
    ...
    def __init__(
        ...
        if self.indices.size < 2:
            raise ValueError("Must have at least 2 support points")
        if not np.all(np.diff(self.indices) > 0):
            raise ValueError("indices must be strictly increasing")
        if self.indices.shape != np.asarray(PMF_array).shape:
            raise ValueError("indices and PMF_array must have same shape")
        super().__init__(x_min=x_min, x_gap=x_gap, PMF_array=PMF_array, p_neg_inf=p_neg_inf, p_pos_inf=p_pos_inf)
    ...
    def get_sparsity(self) -> float:
        span = int(self.indices[-1] - self.indices[0] + 1)
        return 1.0 - (self.indices.size / span)
    ...
    def _create_truncated(
        ...
        return SparseLinearDiscreteDist(
            x_min=self.x_min,
            x_gap=self.x_gap,
            indices=self.indices[min_ind : max_ind + 1],
            PMF_array=new_PMF,
            p_neg_inf=new_p_neg_inf,
            p_pos_inf=new_p_pos_inf,
        )

    def copy(self) -> SparseLinearDiscreteDist:
        return SparseLinearDiscreteDist(
            x_min=self.x_min,
            x_gap=self.x_gap,
...
# =============================================================================
# STRUCTURED GEOMETRIC DISTRIBUTIONS
# =============================================================================


class GeometricDiscreteDistBase(DiscreteDistBase, ABC):
    """Abstract base for geometric grids x[k] = x_min * ratio^index[k]."""
    ...
    @abstractmethod
    def get_indices(self) -> NDArray[np.int64]:
        """Return integer indices that map support to x_min * ratio^indices."""

    def get_x_array(self) -> NDArray[np.float64]:
        return self.x_min * np.power(self.ratio, self.get_indices().astype(np.float64))

    def get_support_size(self) -> int:
        return self.get_indices().size


class DenseGeometricDiscreteDist(GeometricDiscreteDistBase):
    """Dense geometric grid x[i] = x_min * ratio^i."""
    ...
    def __init__(
        ...
        super().__init__(x_min=x_min, ratio=ratio, PMF_array=PMF_array, p_neg_inf=p_neg_inf, p_pos_inf=p_pos_inf)
    ...
    def from_x_array(
        ...
        """Create DenseGeometricDiscreteDist from x_array by extracting x_min and ratio.
        ...
        Returns:
            DenseGeometricDiscreteDist instance
    ...
    def copy(self) -> DenseGeometricDiscreteDist:
        return DenseGeometricDiscreteDist(
            x_min=self.x_min,
            ratio=self.ratio,
            PMF_array=self.PMF_array.copy(),
...
class SparseGeometricDiscreteDist(GeometricDiscreteDistBase):
    """Sparse geometric grid x[k] = x_min * ratio^(indices[k])."""
    ...
    def copy(self) -> SparseGeometricDiscreteDist:
        return SparseGeometricDiscreteDist(
            x_min=self.x_min,
            ratio=self.ratio,
            indices=self.indices.copy(),
