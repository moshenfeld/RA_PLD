...
def _log_exp_space_with_target_gap(
    exp_dist: DiscreteDistBase,
    target_loss_gap: float,
    bound_type: BoundType,
) -> LinearDiscreteDistBase:
    """
    Convert exp-space support to loss-space while targeting a specific linear loss gap.

    For non-geometric exp-space inputs (e.g., FFT linear grids), first regrid to geometric
    with log-step `target_loss_gap`, then apply typed log transform.
    """
    if isinstance(exp_dist, GeometricDiscreteDistBase):
        return log_geometric_to_linear(exp_dist)

    exp_geom = change_spacing_type(
        dist=exp_dist,
        tail_truncation=0.0,
        loss_discretization=target_loss_gap,
        spacing_type=SpacingType.GEOMETRIC,
        bound_type=bound_type,
    )
    assert isinstance(exp_geom, GeometricDiscreteDistBase)
...
def _maybe_sparsify_geometric_dist(
    dist: DenseGeometricDiscreteDist,
    use_sparse: bool,
) -> GeometricDiscreteDistBase:
    """Convert exact-zero dense geometric PMFs to sparse form when requested."""
    if not use_sparse:
        return dist

    keep = dist.PMF_array != 0.0
    keep_count = int(np.count_nonzero(keep))
    if keep_count < 2 or keep_count == dist.PMF_array.size:
        return dist
...
def _allocation_PMF_remove_fft(
    ...
    assert isinstance(base_dist_lower, DenseLinearDiscreteDist)
    shifted_lower = shift_distribution(base_dist_lower, -lower_shift)
    assert isinstance(shifted_lower, DenseLinearDiscreteDist)
    ...
    assert isinstance(shifted_upper, DenseLinearDiscreteDist)
    ...
    return _log_exp_space_with_target_gap(
        exp_dist=shift_distribution(
            conv_dist_raw,
            (num_steps - 1) * lower_shift + upper_shift,
        ),
        target_loss_gap=conv_params.pre_composition_loss_discretization,
        bound_type=bound_type,
    )
...
def _allocation_PMF_remove_geom(
    ...
    assert isinstance(base_dist_lower, DenseGeometricDiscreteDist)
    sparse_ready_lower = _maybe_sparsify_geometric_dist(base_dist_lower, use_sparse=use_sparse)
    conv_dist_lower = geometric_self_convolve(
        dist=sparse_ready_lower,
        T=num_steps - 1,
        tail_truncation=pre_composition_tail_truncation,
        bound_type=bound_type,
    )
    ...
    assert isinstance(base_dist_upper, DenseGeometricDiscreteDist)
    sparse_ready_upper = _maybe_sparsify_geometric_dist(base_dist_upper, use_sparse=use_sparse)

    conv_dist_raw = geometric_convolve(
        dist_1=conv_dist_lower,
        dist_2=sparse_ready_upper,
        tail_truncation=pre_composition_tail_truncation,
        bound_type=bound_type,
    )
    return _log_exp_space_with_target_gap(
        exp_dist=conv_dist_raw,
        target_loss_gap=conv_params.pre_composition_loss_discretization,
        bound_type=bound_type,
    )
...
def _allocation_PMF_add_fft(
    ...
    assert isinstance(base_dist, DenseLinearDiscreteDist)
    ...
    log_dist = _log_exp_space_with_target_gap(
        exp_dist=conv_dist,
        target_loss_gap=conv_params.pre_composition_loss_discretization,
        bound_type=opposite_bound_type,
    )
...
def _allocation_PMF_add_geom(
    ...
    assert isinstance(base_dist, DenseGeometricDiscreteDist)
    sparse_ready_base = _maybe_sparsify_geometric_dist(base_dist, use_sparse=use_sparse)
    conv_dist = geometric_self_convolve(
        dist=sparse_ready_base,
        T=num_steps,
        tail_truncation=pre_composition_tail_truncation,
        bound_type=opposite_bound_type,
    )
...
def _finalize_allocation_dist(
    ...
    if not (
        isinstance(selected_dist, DenseLinearDiscreteDist)
    ...
    assert isinstance(selected_dist, DenseLinearDiscreteDist)
    ...
    assert isinstance(final_dist, DenseLinearDiscreteDist)
    return final_dist
...
def _allocation_PMF(conv_params: _ConvParams,
    ...
    if convolution_method in (ConvolutionMethod.FFT, ConvolutionMethod.BEST_OF_TWO):
        FFT_based_dist = conv_func(conv_params=conv_params,
                                   bound_type=bound_type,
                                   convolution_method=ConvolutionMethod.FFT,
                                   use_sparse=False)  # FFT doesn't use sparse
    if convolution_method in (ConvolutionMethod.GEOM, ConvolutionMethod.BEST_OF_TWO):
        geom_based_dist = conv_func(conv_params=conv_params,
                                    bound_type=bound_type,
                                    convolution_method=ConvolutionMethod.GEOM,
                                    use_sparse=use_sparse)  # Use sparse for geometric
...
def allocation_PLD(
    ...
    assert isinstance(remove_dist, LinearDiscreteDistBase)
    ...
    assert isinstance(add_dist, LinearDiscreteDistBase)
